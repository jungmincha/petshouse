						...
						<div id="input" class="row">
							...
							<div class="col-lg-6">
								<div class="product-pic-zoom">
									<img class="product-big-img" height="550"
										src="/resources/img/admin/goods/${board.goodsVO.thumbnail}"
										alt="">
									<div class="zoom-icon">
								...
								<div class="product-thumbs">
									<div class="product-thumbs-track ps-slider owl-carousel">
										<div class="pt active"
											data-imgbigurl="/resources/img/admin/goods/${board.goodsVO.thumbnail}">
											<img
												src="/resources/img/admin/goods/${board.goodsVO.thumbnail}"
												width="1270" alt="">
										</div>
										<c:forEach var="goods" items="${img}">
											<div class="pt"
												data-imgbigurl="/resources/img/admin/board/${board.imgname}">
												<img src="/resources/img/admin/board/${board.imgname}"
													alt="">
											</div>
										</c:forEach>
									</div>
							...
							<div class="col-lg-6">
								<div class="product-details">
									<div class="pd-title">
										<input type="hidden" name="board_id" value="${gooboardds.board_id}">
										<input type="hidden" name="goods_id"
											value="${board.goodsVO.goods_id}"> <input
											type="hidden" name="goods_id" value="${board.pdate}">
										<span>${board.categoryVO.categoryname}/${board.categoryVO.sortname}</span>
										<h3>${board.goodsVO.goodsname}</h3>
									...
									<div class="pd-rating">
										...
										<c:if test="${one.goodsVO.goods_id eq board.goodsVO.goods_id}">
									...
									<div class="pd-desc">
										<small>${board.goodsVO.description}</small>
										<h4>
											<fmt:formatNumber value="${board.goodsVO.price}"
												pattern="###,###,###" />
											원
										</h4>
									...
									<div class="pd-color">
										<h6>Color ${board.goodsVO.pcolor} 뿌릴예정</h6>
									...
									<div class="pd-size-choose">
										<c:set var="pcolor" value="${board.goodsVO.pcolor}" />
										<c:set var="color" value="${fn:split(pcolor, ',')}" />
										<c:forEach var="c" items="${color}">
											<div class="sc-item">
												<input type="radio" id="s-size" /> <label for="s-size">${c}</label>
											</div>
										</c:forEach>
									</div>
									...
									<div class="pd-size-choose">
										<c:set var="psize" value="${board.goodsVO.psize}" />
										<c:set var="size" value="${fn:split(psize, ',')}" />
										<c:forEach var="s" items="${size}">
											<div class="sc-item">
												<input type="radio" id="lg-size" /> <label for="lg-size">${s}</label>
											</div>
										</c:forEach>
									</div>
									...
									<ul class="pd-tags">
										...
										<li><span>TAGS</span>: <input type="text"
											class="form-control" name="hashtag" value="${board.hashtag}"
											id="hashtag" aria-describedby="hashtagHelp" /> <small
											id="hashtagHelp" class="form-text text-muted"> 예)
												#강아지 #고양이 </small></li>
									</ul>
									<div class="pd-share">
										<div class="p-code">
											<small>재고상태 : ${board.goodsVO.stockVO.stockname}</small>
										</div>
